...
const FriendsList: React.FC<FriendsListProps> = ({ clickOnUser }) => {
  ...
  const fetchFriends = async (myId: number) => {
    ...
    try {
      const response = await axiosInstance.get<User[]>(
        `http://jemoederinator.local:3001/friendship/${myId}/friends`
      );
  ...
  // 	try {
  // 		await axiosInstance.delete("http://jemoederinator.local:3001/users/");
              ...
              .map((friend) => (
                <ListGroup.Item key={friend.id}>
                  ...
                  <span>
                    ...
                    >
                      <img
                        src={"http://jemoederinator.local:3001/" + friend.profileImage}
