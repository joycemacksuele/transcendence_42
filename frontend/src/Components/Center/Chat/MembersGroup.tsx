...
import {
  CurrentUserContext,
  CurrUserData,
} from "../Profile_page/contextCurrentUser.tsx";
...
const MembersGroup: React.FC<PropsHeader> = ({ chatClicked }) => {
  ...
  const intraName =
    currUserData.loginName === undefined
      ? "your friend"
      : currUserData.loginName;

  const joinGroupChat = () => {
    console.log(
      "[MembersGroup] Will join the chat",
      chatClicked?.name,
      "id",
      chatClicked?.id
    );
    ...
    <>
      ...
      <Row className="h-20 align-items-bottom">
        <Stack gap={2} className="align-self-center">
          ...
          {chatClicked?.users.indexOf(intraName) != -1 && (
            <Button variant="warning" onClick={leaveGroupChat}>
              Leave group
            </Button>
          )}
          ...
          {chatClicked?.users.indexOf(intraName) == -1 &&
            chatClicked?.type != ChatType.PROTECTED && (
              <Button variant="primary" onClick={joinGroupChat}>
                Join group
              </Button>
            )}
